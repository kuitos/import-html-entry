...
function getExecutableScript(scriptSrc, scriptText, opts = {}) {
  ...
  // 将 scopedGlobalVariables 拼接成函数声明，用于缓存全局变量，避免每次使用时都走一遍代理
	...
	return strictGlobal
		? (
      scopedGlobalVariableFormalParams
				? `;with(window.proxy){(function(${scopedGlobalVariableFormalParams}){;${scriptText}\n${sourceUrl}}).bind(window)(${scopedGlobalVariableActualParams})};`
