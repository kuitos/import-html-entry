...
function importHTML(url) {
  ...
  return embedHTMLCache[url] || (embedHTMLCache[url] = fetch(url).then(function (response) {
    ...
    var _ref = (0, _asyncToGenerator2["default"])( /*#__PURE__*/_regenerator["default"].mark(function _callee(html) {
      var assetPublicPath, doc, metas, modules, m, scriptsString, _processTpl, template, scripts, entry, styles, SystemJS, p;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              metas = doc.querySelector('meta[name="autoModules"]');
              modules = null;
              ...
              m = JSON.parse(modules ? modules : '[]');

              if (m.length >= 1) {
                scriptsString = '';
                m[1].forEach(function (item) {
                  scriptsString += '<script src="' + item + '"></script>';
                });
                html = html.replace(/<\/body>/, scriptsString + '</body>');
              }

              _processTpl = (0, _processTpl2["default"])(getTemplate(html), assetPublicPath), template = _processTpl.template, scripts = _processTpl.scripts, entry = _processTpl.entry, styles = _processTpl.styles;
              ...
              m[0].forEach(function (item, i) {
                if (!window[m[0][i]]) {
                  var pi = SystemJS["import"](item);
                  p.push(pi);
                }
              });
              _context.next = 15;
              return Promise.all(p).then(function (list) {
            ...
            case 15:
              return _context.abrupt("return", getEmbedHTML(template, styles, {
                fetch: fetch
